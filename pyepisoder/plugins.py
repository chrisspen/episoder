...
class TVDB(object):
    ...
    def parse(self, show, store, args):
        ...
        if show.url.isdigit():
            ...
            self.logger.error('%s is not a valid TVDB show id', show.url)
            return
        ...
        try:
            ...
            self.logger.error('Show %s not found', _id)
            return
...
class EpguidesParser(object):
    ...
    def parse(self, show, store, _):
        ...
        try:
            ...
            self.logger.error("Error fetching %s: %s", self.show.url, e)
            return
    ...
    def parseFile(self, filename, store):
        self.store = store
        yamlfile = self._runAwk(filename)
    ...
    def _runAwk(self, webdata):
        ...
        cmd = 'iconv -c -f utf8 -t iso-8859-1 %s >%s' % (webdata, cleanwebdata)
        if os.system(cmd) != 0:
        ...
        fin = open(logfile)
        self.logger.debug(fin.read().strip())
        fin.close()
    ...
    def _readYaml(self, yamlfile):
        ...
        fin = open(yamlfile)
        data = yaml.load(fin.read().decode('iso8859-1'))
        fin.close()
...
class ConsoleRenderer(object):
    ...
    def __init__(self, format, dateformat): # pylint: disable=redefined-builtin
